...
class EditParDialog:
    ...
    def __init__(self, theTask, parent=None, isChild=0,
        ...
        self.top = top = tkinter.Toplevel(
            self.parent, bg=self._frmeColor, visual="best")
        ...
        # Create a spacer
        tkinter.Frame(self.top, bg=self._taskColor,
                      height=10).pack(side="top", fill="x")
        ...
        tkinter.Frame(self.top, bg=self._taskColor,
                      height=15).pack(side="top", fill="x")

        # Set control buttons at the top of the frame
        self.buttonBox(self.top)

        # Insert a spacer between the static text and the buttons
        tkinter.Frame(self.top, bg=self._entsColor,
                      height=15).pack(side="top", fill="x")
...
# instead in the event of such errors.
    ...
    def _setupDefaultParamList(self):
        ...
        # for use below
        ourpardict = {}
        for par in theParamList:
            ourpardict[par.fullName()] = par
    ...
    def printNames(self, top, taskName, pkgName):
        ...
        topbox = tkinter.Frame(top, bg=self._taskColor)
        textbox = tkinter.Frame(topbox, bg=self._taskColor)
#       helpbox = tkinter.Frame(topbox, bg=self._taskColor)
    ...
    def makeFileMenu(self, menubar):
        ...
        fileButton = tkinter.Menubutton(
            menubar, text='File', bg=self._frmeColor)
...
#       fileButton.menu.add_command(label="Open...", command=self.pfopen)
    ...
    def makeOpenMenu(self, menubar):
        ...
        self._openMenuChoice = tkinter.StringVar()  # this is used till GUI closes
        self._numOpenMenuItems = 1  # see dummy

        openBtn = tkinter.Menubutton(
            menubar, text='Open...', bg=self._frmeColor)
        openBtn.bind("<Enter>", self.printOpenInfo)
        openBtn.pack(side="left", padx=2)

        openBtn.menu = tkinter.Menu(
            openBtn, tearoff=0, postcommand=self._updateOpen)
    ...
    def makeOptionsMenu(self, menubar):
        ...
        self._helpChoice = tkinter.StringVar()
        ...
        if self._showSaveCloseOnExec:
            self._execChoice = tkinter.IntVar()
            self._execChoice.set(int(self._saveAndCloseOnExec))

        optionButton = tkinter.Menubutton(
            menubar, text="Options", bg=self._frmeColor)
        ...
        if self._showFlaggingChoice:
            self._flagChoice = tkinter.IntVar()
...
#           print('LAUNCHING: '+url) # DBG
    ...
    def makeHelpWin(self, helpString, title="Parameter Editor Help Browser"):
        ...
        hb = tkinter.Toplevel(self.top, bg=None)
        hb.title(title)
        hb.iconLabel = title
    ...
    def setAllEntriesFromParList(self, aParList, updateModel=False):
        ...
        for i in range(self.numParams):
            ...
            if isinstance(aParList, list):
                ...
                for newpar in aParList:
                    if newpar.name == par.name and newpar.scope == par.scope:
                        # same as .get(native=1,prompt=0)
                        par.set(newpar.value)
                        found = True
                        break
            ...
            gui_entry.forceValue(
                par.value, noteEdited=False)  # no triggers yet
