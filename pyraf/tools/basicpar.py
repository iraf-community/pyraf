...
for field in flist:
    _getFieldDict.add(field, field)

flist = ("p_prompt", "p_value", "p_filename",
         "p_maximum", "p_minimum", "p_mode", "p_scope")
_setFieldDict = minmatch.MinMatchDict()
for field in flist:
    _setFieldDict.add(field, field)
del flist, field
...
class IrafPar:
    ...
    def getWithPrompt(self):
        ...
        if self.value is not None:
            pstring = pstring + \
                " (" + self.toString(self.value, quoted=1) + ")"
        pstring = pstring + ": "
    ...
    def pretty(self, verbose=0):
        ...
        if self.type in alwaysquoted and self.value is not None:
            pvalue = '"' + pvalue + '"'
        if self.mode == "h":
...
def _getChoice(s, strict):
    ...
    if not clist[0]:
        del clist[0]
    if len(clist) > 1 and not clist[-1]:
        del clist[-1]
    return clist
