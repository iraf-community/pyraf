...
C =======================================================================
	...
	integer		i, nx, ny, x, y, x2, y2, fill, size, color
...
C           -------------
		...
		call cfmarkpoint (x, y, i, size, M_PLUS, color, ier)
	    else if (key .eq. 'x') then
		call cfmarkpoint (x, y, i, size, M_CROSS, color, ier)
	    else if (key .eq. '.') then
		call cfmarkpoint (x, y, i, size, M_POINT, color, ier)
	    else if (key .eq. '*') then
		call cfmarkpoint (x, y, i, size, M_STAR, color, ier)
	    else if (key .eq. '_') then
		call cfmarkpoint (x, y, i, size, M_HBLINE, color, ier)
	    else if (key .eq. '|') then
		call cfmarkpoint (x, y, i, size, M_VBLINE, color, ier)
	    else if (key .eq. 'o') then
		call cfmarkpoint (x, y, i, size, ior(M_CIRCLE,fill),
     &		    color, ier)
	    else if (key .eq. 's') then
		call cfmarkpoint (x, y, i, size, ior(M_BOX,fill), color,
     &		    ier)
	    else if (key .eq. 'v') then
		call cfmarkpoint (x, y, i, size, ior(M_DIAMOND,fill),
     &		    color, ier)
